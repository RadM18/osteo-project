...
function additional_mime_types( $mimes ) {
	$mimes['svg'] = 'application/scalable_vector_graphics';
	return $mimes;
}
add_filter( 'upload_mimes', 'additional_mime_types' );

if(function_exists('acf_add_options_page')){
    acf_add_options_page('Расписание');
    acf_add_options_page('Контакты в шапку');
    acf_add_options_page('Меню курсов');
}

function delete_code_editor_item() {
  remove_action('admin_menu', '_add_themes_utility_last', 101);
}
add_action('_admin_menu', 'delete_code_editor_item', 1);
